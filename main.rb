...
# Формат каталога:
#   Раздел | Подраздел | Наименование | Адрес карточки | Адрес картинки
class PiknikParser
  ...
  def initialize(path=nil, sep=nil, img_dir=nil)
    ...
    # Храним ссылки на карточки, чтобы отсекать дубли
    @saved = @catalog_file.readlines.collect{ |r| r[2] }
  ...
  def parse_subsect(subsect_page, sect, subsect)
    subsect_page.search(GOODSCARD_SEL).each do |goods_link|
      ...
      next if @saved.index(href)
      @catalog_file << [sect, subsect, name, href, img_url]
      puts "    #{ name }"

      unless img_url.empty?
        img_name = "#{ @img_dir }/#{ hash }#{ File.extname(img_url) }"
        @mechanize.get(MAIN_URL + img_url).save(img_name)
      end
    ...
    if next_link
      next_href = next_link.attributes['href']
      puts "Go to page #{ next_href }"
      parse_subsect(@mechanize.get(MAIN_URL + '/' + next_href), sect, subsect)
    end
  end

  def get_row_by_hash(hash)
    return [] unless @saved.index(hash)
    CSV.open(@path, 'r', {:col_sep => @sep}).readlines.select{|r| r[5] == hash}.first
  end
